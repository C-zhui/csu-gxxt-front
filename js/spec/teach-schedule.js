require(['jquery', 'lodash', 'api/apiobj', 'config/global', 'util/cut_page3', 'config/config-course-schedule', 'util/date_format_transform', 'api/batch', 'api/student', 'api/experiment', 'api/proced'], function ($, _, api, g, CutPage, ccs, dft) {
  ...
  $batch_selector.change(update_stud_group_selector);
  function update_stud_group_selector() {
    ...
    console.log(div_batches[batch_idx].batch_name)
  ...
  $('#query_by_stud_id').click(get_a_student_by_id);
  function get_a_student_by_id() {
    var sid = $('#stud_id').val();
    if (!sid) return;
